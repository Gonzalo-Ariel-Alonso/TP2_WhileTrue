#include "Lista.h"
#include "Escritor.h"
//Pre:-
//Post: Muestra el menu del programa

void mostrar_menu();

void quitar_lectura_de_una_lista(Lista<Escritor> * lista_de_escritores);


void crear_lista_escritores(Lista<Escritor> * lista_de_escritores);

void listar_escritores(Lista<Escritor> * lista_escritores);

void agregar_escritor(Lista<Escritor> *Lista_de_escritores);

void mostrar_nombre_y_fallecimiento_escritor(Lista<Escritor>* lista_de_escritores);

void modificar_ano_fallecimiento_escritor(Lista<Escritor> * lista_de_escritores);

//Lectura sortear_una_lectura(Lista<Escritor> * lista_de_escritores);

void listar_novelas_por_genero(Lista<Escritor> * lista_de_escritores);

void listar_lecturas(Lista<Escritor> * lista_de_escritores);

void cambiar_dato_escritor(Lista<Escritor> * lista_de_escritores);

